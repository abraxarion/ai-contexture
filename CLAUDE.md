...
Builds `ai-contexture` — a Claude Code skill that routes each project
...
- Get approval before changing a target project's context files.
- Judge context by expected total session cost, not `CLAUDE.md` size.
- Ship no manifest, metadata, telemetry, or compiler in v1.

## Where things live

- Intent: `docs/superpowers/specs/2026-08-25-ai-contexture-v1-design.md`.
  Authoritative — change it before changing behavior.
- Execution: `docs/superpowers/plans/2026-08-25-ai-contexture-v1-implementation-plan.md`.
- Product: `skills/ai-contexture/` — `SKILL.md` plus `references/`. Edit it there;
  `.claude/skills/ai-contexture/` is a gitignored copy, refreshed by copying.
